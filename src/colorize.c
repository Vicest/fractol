...
/*   Updated: 2021/07/02 19:07:45 by vicmarti         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	t_rgb				color_channel;
	const double		range_size = max_iter / 4;
	...
	{
		color_channel.g = 255;
		color_channel.b = 255 - ft_min(((unsigned int)(scale * iterations)), 0xFF);
	}
	else if (iterations < 3 * range_size)
	{
		color_channel.r = ft_min(((unsigned int)(scale * iterations)), 0xFF);
		color_channel.g = 255;
	}
	else if (iterations < 4 * range_size)
	{
		color_channel.r = 255;
		color_channel.g = 255 - ft_min(((unsigned int)(scale * iterations)), 0xFF);
	}
